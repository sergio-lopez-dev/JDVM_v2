...
<template>
  <div class="mx-auto flex w-full max-w-3xl flex-1 flex-col">
    ...
    <div v-if="appt && isBlock" class="flex flex-1 flex-col items-center justify-center px-8 text-center">
      <div class="border-default bg-muted mb-5 flex size-16 items-center justify-center rounded-full border"><UIcon name="i-lucide-lock" class="text-dimmed size-7" /></div>
      <h1 class="font-display text-3xl">Hueco bloqueado</h1>
      <p class="text-toned mt-2 text-sm">{{ fmtDate(appt.startsAt, "EEEE d 'de' MMMM") }} · {{ fmtDate(appt.startsAt, 'HH:mm') }}–{{ fmtDate(appt.endsAt, 'HH:mm') }}</p>
      <p v-if="appt.note" class="text-dimmed mt-1 text-sm">{{ appt.note }}</p>
      <UButton color="error" variant="soft" size="lg" class="mt-6" icon="i-lucide-lock-open" @click="removeBlock">Quitar bloqueo</UButton>
    </div>
    ...
    <template v-else-if="appt">
      <div class="flex-1 space-y-5 overflow-y-auto px-5 pt-2 pb-28">
        ...
        <div v-if="appt.clientPhone" class="grid grid-cols-2 gap-2.5">
          <a :href="waLink(appt.clientPhone)" target="_blank" rel="noopener" class="flex items-center justify-center gap-2 rounded-2xl border border-[#25D366]/40 bg-[#25D366]/10 p-4 text-sm font-semibold text-[#25D366] transition hover:bg-[#25D366]/20">
            <UIcon name="i-lucide-message-circle" class="size-5" />WhatsApp
          </a>
          <a :href="telLink(appt.clientPhone)" class="border-default bg-muted text-toned hover:bg-elevated flex items-center justify-center gap-2 rounded-2xl border p-4 text-sm font-semibold transition">
      ...
      <div class="border-default bg-default sticky bottom-0 flex gap-3 border-t px-5 py-4">
        ...
        <UButton v-else color="neutral" variant="soft" size="lg" class="flex-[1.5] justify-center" icon="i-lucide-undo-2" @click="revertToBooked">{{ appt.status === 'no_show' ? 'Deshacer “no vino”' : 'Deshacer “hecha”' }}</UButton>
      </div>
    </template>
