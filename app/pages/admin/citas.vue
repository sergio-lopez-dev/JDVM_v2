...
function statusKind(s: string): PillKind {
  return s === 'completed' ? 'done' : s === 'cancelled' ? 'cancelled' : s === 'no_show' ? 'neutral' : 'confirmed'
}
function statusLabel(s: string) {
  return s === 'completed' ? 'Hecha' : s === 'cancelled' ? 'Cancelada' : s === 'no_show' ? 'No vino' : 'Confirmada'
}
...
const filtered = computed(() => {
  ...
  return enriched.value
    ...
    .filter((a) =>
      !q ? true : a.clientName.toLowerCase().includes(q) || a.serviceName.toLowerCase().includes(q) || a.barberName.toLowerCase().includes(q),
    )
...
const pageRows = computed(() => filtered.value.slice((page.value - 1) * perPage, page.value * perPage))
...
<template>
  <div>
    <AdminHeader v-model:search="search" title="Citas" sub="Gestiona y revisa todas las reservas" show-search search-placeholder="Buscar cliente, servicio…">
      <template #actions>
    ...
    <div class="space-y-4 px-5 py-6 pb-24 lg:px-7 lg:pb-6">
      ...
      <div class="space-y-2.5 lg:hidden">
        ...
        >
          <div class="flex items-center gap-3">
            <span class="bg-elevated border-default flex size-9 shrink-0 items-center justify-center rounded-full border text-xs font-semibold">{{ initials(a.clientName) }}</span>
          ...
          <div class="border-default mt-3 flex items-center gap-4 border-t pt-3">
            <span class="text-toned flex items-center gap-1.5 font-mono text-xs"><UIcon name="i-lucide-clock" class="text-dimmed size-3.5" />{{ fmtDate(a.startsAt, 'HH:mm') }} · {{ dayLabel(a.startsAt) }}</span>
            <span class="text-toned flex items-center gap-1.5 text-xs"><UIcon name="i-lucide-user" class="text-dimmed size-3.5" />{{ a.barberName }}</span>
        ...
        <div v-if="!filtered.length" class="border-default rounded-2xl border p-8">
          <UiEmptyState icon="i-lucide-calendar-x" title="Sin citas" description="No hay citas que coincidan." />
        </div>
      </div>
      ...
      <AdminCard :pad="false" class="hidden lg:block">
        <div class="overflow-x-auto">
          <div class="min-w-[760px]">
            <div class="bg-accented border-default text-dimmed grid grid-cols-[110px_2fr_1.4fr_1.4fr_80px_120px_40px] gap-3 border-b px-5 py-3 font-mono text-[0.6rem] tracking-widest uppercase">
              <span>Hora</span><span>Cliente</span><span>Servicio</span><span>Barbero</span><span>Precio</span><span>Estado</span><span />
            </div>
            <button
              v-for="a in pageRows"
              :key="a.id"
            ...
            >
              <div>
                <div class="font-mono text-sm font-semibold">{{ fmtDate(a.startsAt, 'HH:mm') }}</div>
                <div class="text-dimmed text-[0.7rem]">{{ dayLabel(a.startsAt) }}</div>
              </div>
              <div class="flex min-w-0 items-center gap-3">
                <span class="bg-elevated border-default flex size-8 shrink-0 items-center justify-center rounded-full border text-[0.65rem] font-semibold">{{ initials(a.clientName) }}</span>
            ...
            <div v-if="!filtered.length" class="p-10">
              <UiEmptyState icon="i-lucide-calendar-x" title="Sin citas" description="No hay citas que coincidan con el filtro." />
            </div>
          </div>
        </div>
      </AdminCard>
      ...
      <div v-if="filtered.length" class="flex items-center justify-between">
        <span class="text-dimmed text-xs">Mostrando {{ pageRows.length }} de {{ filtered.length }} citas</span>
        <div class="flex items-center gap-1.5">
          <button type="button" aria-label="Anterior" class="border-default bg-muted hover:bg-elevated flex size-9 items-center justify-center rounded-[10px] border disabled:opacity-40" :disabled="page <= 1" @click="page--"><UIcon name="i-lucide-chevron-left" class="size-4" /></button>
          <span class="text-toned px-2 font-mono text-sm">{{ page }} / {{ pages }}</span>
          <button type="button" aria-label="Siguiente" class="border-default bg-muted hover:bg-elevated flex size-9 items-center justify-center rounded-[10px] border disabled:opacity-40" :disabled="page >= pages" @click="page++"><UIcon name="i-lucide-chevron-right" class="size-4" /></button>
        </div>
      </div>
    </div>
    ...
    <Transition enter-active-class="transition-opacity" leave-active-class="transition-opacity" enter-from-class="opacity-0" leave-to-class="opacity-0">
      <div v-if="selected" class="fixed inset-0 z-50 flex items-end justify-center sm:items-center" @click="selected = null">
        ...
        <div class="border-default bg-default relative w-full max-w-md overflow-hidden rounded-t-3xl border p-5 sm:rounded-3xl" @click.stop>
          <div class="flex items-start justify-between">
            <div>
              <p class="text-dimmed font-mono text-[0.6rem] tracking-widest uppercase capitalize">{{ fmtDate(selected.startsAt, "EEE d MMM · HH:mm") }}</p>
              <h2 class="font-display mt-1 text-2xl">{{ selected.clientName }}</h2>
              <p class="text-muted mt-0.5 text-sm">{{ selected.serviceName }} · {{ selected.barberName }}</p>
            </div>
          ...
          <div class="mt-5 grid grid-cols-2 gap-2.5">
            ...
            <UButton v-if="selected.status === 'completed'" color="neutral" variant="soft" block icon="i-lucide-undo-2" @click="revertToBooked">Deshacer “hecha”</UButton>
