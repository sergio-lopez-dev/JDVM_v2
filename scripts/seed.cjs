...
async function run() {
  for (const s of services) {
    const { id, ...data } = s
    await db.collection('services_v2').doc(id).set({ ...data, isPrivate: false }, { merge: true })
  }
  ...
  for (const b of barbers) {
    ...
    await db.collection('barbers_v2').doc(uid).set(
      {
        name: b.name,
        slug: b.slug,
        photoUrl: '',
  ...
  for (const [id, data] of rewards) {
    await db.collection('rewards_v2').doc(id).set({ ...data, createdAt: FieldValue.serverTimestamp() }, { merge: true })
  }

  const adminUid = await ensureUser('admin@jdvm.test', 'Admin JDVM')
  await db.collection('users_v2').doc(adminUid).set(
  ...
  await db.collection('users_v2').doc(clientUid).set(
  ...
  await db.collection('appointments_v2').doc('seed-up').set(
  ...
  await db.collection('appointments_v2').doc('seed-past').set(
  ...
  for (let i = 1; i <= 11; i++) {
    ...
    await db.collection('appointments_v2').doc('seed-hist-' + i).set(
  ...
  for (const [id, data] of seedReviews) {
    await db.collection('reviews_v2').doc(id).set(
